...
from utils import (
    init_bool, init_string, init_int, init_double, UserInteraction
)
# if not is_macos:
...
class ComboBoxEditableMenu(ttk.Combobox):
    def __init__(self, master=None, pattern=None, default=None, width=None, is_stay_disabled=False, **kw):
        ...
        self.textvariable = kw.get('textvariable', init_string(""))
        self.pattern = pattern
        self.test = 1
        self.tooltip = ToolTip(self)
        self.is_user_input_var = init_bool(value=False)
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def __init__(self, cli=False):
        self.cli = cli
        self.gui = not cli
        self.inter = UserInteraction(self)
        ...
        if self.gui:
            ...
            self.error_log_var = init_string(value='')
        ...
        try:
            ...
            self.inter.log_error(error_text('Loading Saved Variables', e))
        ...
        self.progress_bar_main_var = init_int(value=0)
        self.inputPathsEntry_var = init_string(value='')
        self.conversion_Button_Text_var = init_string(value=START_PROCESSING)
        self.chosen_ensemble_var = init_string(value=CHOOSE_ENSEMBLE_OPTION)
        self.ensemble_main_stem_var = init_string(value=CHOOSE_STEM_PAIR)
        self.ensemble_type_var = init_string(value=MAX_MIN)
        self.save_current_settings_var = init_string(value=SELECT_SAVED_SET)
        self.demucs_stems_var = init_string(value=ALL_STEMS)
        self.mdxnet_stems_var = init_string(value=ALL_STEMS)
        self.is_primary_stem_only_Text_var = init_string(value='')
        self.is_secondary_stem_only_Text_var = init_string(value='')
        self.is_primary_stem_only_Demucs_Text_var = init_string(value='')
        self.is_secondary_stem_only_Demucs_Text_var = init_string(value='')
        self.scaling_var = init_double(value=1.0)
        self.active_processing_thread = None
        self.verification_thread = None
        self.is_menu_settings_open = False
        self.is_root_defined_var = init_bool(value=False)
        self.is_check_splash = False

        self.is_open_menu_advanced_vr_options = init_bool(value=False)
        self.is_open_menu_advanced_demucs_options = init_bool(value=False)
        self.is_open_menu_advanced_mdx_options = init_bool(value=False)
        self.is_open_menu_advanced_ensemble_options = init_bool(value=False)
        self.is_open_menu_view_inputs = init_bool(value=False)
        self.is_open_menu_help = init_bool(value=False)
        self.is_open_menu_error_log = init_bool(value=False)
        self.is_open_menu_advanced_align_options = init_bool(value=False)
        ...
        self.progress_bar_var = init_int(value=0)
        self.is_confirm_error_var = init_bool(value=False)
        ...
        self.model_download_demucs_var = init_string(value='')
        self.model_download_mdx_var = init_string(value='')
        self.model_download_vr_var = init_string(value='')
        self.selected_download_var = init_string(value=NO_MODEL)
        self.select_download_var = init_string(value='')
        self.download_progress_info_var = init_string(value='')
        self.download_progress_percent_var = init_string(value='')
        self.download_progress_bar_var = init_int(value=0)
        self.download_stop_var = init_string(value='')
        self.app_update_status_Text_var = init_string(value=LOADING_VERSION_INFO_TEXT)
        self.app_update_button_Text_var = init_string(value=CHECK_FOR_UPDATES_TEXT)

        self.user_code_validation_var = init_string(value='')
        self.download_link_path_var = init_string(value='')
        self.download_save_path_var = init_string(value='')
        self.download_update_link_var = init_string(value='')
        self.download_update_path_var = init_string(value='')
        ...
        self.file_one_sub_var = init_string(value=FILE_ONE_MAIN_LABEL)
        self.file_two_sub_var = init_string(value=FILE_TWO_MAIN_LABEL)
        ...
        if self.gui:
            ...
            self.inter.write_command_text(f'Ultimate Vocal Remover {VERSION} [{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]')
    ...
    def bind_widgets(self):
        ...
        self.chosen_audio_tool_align = init_bool(value=True)
    ...
    def delete_temps(self, is_start_up=False):
        ...
        try:
            ...
            self.inter.log_error(error_text(TEMP_FILE_DELETION_TEXT, e))
    ...
    def check_is_menu_open(self, menu):
        try:
            ...
            self.inter.log_error("{}".format(error_text(menu, e)))
            ...
            if os.path.isfile(i):
                try:
                    ...
                    if is_process:
                        ...
                        self.inter.log_error(f'{ERROR_LOADING_FILE_TEXT[0]}:\n\n\"{audio_base_name}\"\n\n{ERROR_LOADING_FILE_TEXT[1]}:\n\n{message}')
                    else:
    ...
    def menu_view_inputs(self):
        ...
        input_length_var = init_string(value='')
        input_info_text_var = init_string(value='')
        is_widen_box_var = init_bool(value=False)
        is_play_file_var = init_bool(value=False)
        varification_text_var = init_string(value=VERIFY_INPUTS_TEXT)
        ...
        def verify_audio(is_create_samples=False):
            ...
            if iterated_list:
                ...
                if removed_files:
                    ...
                    removed_files = list_diff(self.inputPaths, inputPaths)
                    [input_files_listbox_Option.delete(input_files_listbox_Option.get(0, tk.END).index(i)) for i in removed_files]
                    self.inter.log_error(REMOVED_FILES(list_to_string(removed_files), error_text))
                    self.inputPaths = tuple(inputPaths)
                    self.update_inputPaths()
                else:
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):#**
        ...
        option_var = init_string(value=SELECT_SAVED_SETTING)
        ...
        self.main_menu_var = init_string(value=CHOOSE_ADVANCED_MENU_TEXT)
    ...
    def menu_advanced_ensemble_options(self):#**
        ...
        option_var = init_string(value=SELECT_SAVED_ENSEMBLE)
    ...
    def menu_secondary_model(self, tab, ai_network_vars: dict):
        ...
        voc_inst_secondary_model_scale_LABEL_var = init_string(value=init_convert_to_percentage(voc_inst_secondary_model_scale_var.get()))
        other_secondary_model_scale_LABEL_var = init_string(value=init_convert_to_percentage(other_secondary_model_scale_var.get()))
        bass_secondary_model_scale_LABEL_var = init_string(value=init_convert_to_percentage(bass_secondary_model_scale_var.get()))
        drums_secondary_model_scale_LABEL_var = init_string(value=init_convert_to_percentage(drums_secondary_model_scale_var.get()))
    ...
    def menu_manual_downloads(self):
        ...
        model_selection_var = init_string(value=SELECT_MODEL_TEXT)
        #info_text_var = init_string(value='')
    ...
    def pop_up_save_current_settings(self):
        ...
        settings_save_var = init_string(value='')
    ...
    def pop_up_update_confirmation(self):
        ...
        is_download_in_app_var = init_bool(value=False)
    ...
    def pop_up_change_model_defaults(self, top_window):
        ...
        change_model_defaults_var = init_string(value=NO_MODEL)
    ...
    def pop_up_mdx_model(self, mdx_model_hash, model_path):
        ...
        try:
            ...
            if is_ckpt:
                ...
                self.inter.log_error("{}".format(error_text('MDX-Net Model Settings', e)))
                self.mdx_model_params = None

        if is_compatible_model:
            mdx_model_set = tk.Toplevel(root)
            mdx_n_fft_scale_set_var = init_string(value=n_fft)
            mdx_dim_f_set_var = init_string(value=dim_f)
            mdx_dim_t_set_var = init_string(value=dim_t)
            primary_stem_var = init_string(value=primary_stem)
            mdx_compensate_var = init_string(value=1.035)

            balance_value_var = init_string(value=0)
            is_kara_model_var = init_bool(value=False)
            is_bv_model_var = init_bool(value=False)
    ...
    def pop_up_mdx_c_param(self, mdx_model_hash):
        ...
        mdx_c_model_param_var = init_string(value=NONE_SELECTED)
    ...
    def pop_up_vr_param(self, vr_model_hash):
        ...
        vr_model_param_var = init_string(value=NONE_SELECTED)
        vr_model_stem_var = init_string(value='Vocals')
        vr_model_nout_var = init_string(value=32)
        vr_model_nout_lstm_var = init_string(value=128)
        is_new_vr_model_var = init_bool(value=False)
        balance_value_var = init_string(value=0)
        is_kara_model_var = init_bool(value=False)
        is_bv_model_var = init_bool(value=False)
    ...
    def pop_up_input_stem_name(self, stem_var:tk.StringVar, parent_window:tk.Toplevel):
        ...
        stem_input_save_var = init_string(value='')
        is_inverse_stem_var = init_bool(value=False)
    ...
    def pop_up_save_ensemble(self):
        ...
        ensemble_save_var = init_string(value='')
    ...
    def deletion_entry(self, selection: str, path, callback):
        ...
        if selection not in [SELECT_SAVED_SET, SELECT_SAVED_ENSEMBLE]:
            ...
            confirm = self.inter.message_box(DELETE_ENS_ENTRY)
            if confirm:
    ...
    def online_data_refresh(self, user_refresh=True, confirmation_box=False, refresh_list_Button=False, is_start_up=False, is_download_complete=False):
        ...
        def online_check():
            ...
            try:
                ...
                if self.lastest_version == current_patch and not is_start_up:
                    ...
                    if not user_refresh:
                        if not is_beta_version and not self.lastest_version == current_patch:
                            self.inter.write_command_text(NEW_UPDATE_FOUND_TEXT(self.lastest_version))
                ...
                try:
                    self.inter.log_error(error_text('Online Data Refresh', e))
                except Exception as e:
                    print(e)
    ...
    def download_item(self, is_update_app=False):
        ...
        def push_download():
            ...
            try:
                ...
                self.inter.log_error(error_text(DOWNLOADING_ITEM, e))
    ...
    def process_storage_check(self):
        ...
        if int(free/1.074e+9) in [3, 4, 5, 6, 7, 8]:
            appropriate_storage = self.inter.message_box([STORAGE_WARNING[0], f'{STORAGE_WARNING[1]}{space_details}{CONFIRM_WARNING}'])
    ...
    def process_initialize(self):
        ...
        if self.cli:
            ...
            self.active_processing_thread = KThread(target=target_function1)
        ...
        if self.thread_check(self.active_processing_thread):
            ...
            if confirm:
                try:
                    ...
                    self.inter.write_command_text(PROCESS_STOPPED_BY_USER)
        else:
    ...
    def process_tool_start(self):
        ...
        def handle_ensemble(inputPaths, audio_file_base):
            ...
            self.inter.write_command_text(DONE)
        ...
        def handle_alignment_match(audio_file, audio_file_base, command_Text, set_progress_bar):
            ...
            self.inter.write_command_text(f"{DONE}\n")
        ...
        if self.chosen_audio_tool_var.get() in [ALIGN_INPUTS, MATCH_INPUTS]:
            if self.DualBatch_inputPaths:
                ...
                if not self.fileOneEntry_Full_var.get() or not self.fileTwoEntry_Full_var.get():
                    self.inter.write_command_text(NOT_ENOUGH_ERROR_TEXT)
                    self.process_end()
                    return
                else:
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                command_Text = lambda text: self.inter.write_command_text(self.base_text + text)
                ...
                if not self.verify_audio(audio_file):
                    ...
                    if total_files >= 2:
                        self.inter.write_command_text(f'\n{error_text_console}')
                ...
                if audio_tool_action not in [MANUAL_ENSEMBLE, ALIGN_INPUTS, MATCH_INPUTS]:
                    ...
                    self.inter.write_command_text(f'{NEW_LINE if file_num != 1 else NO_LINE}{self.base_text}"{os.path.basename(audio_file)}\".{NEW_LINES}')
                    ...
                    if audio_file[0] != audio_file[1]:
                        self.inter.write_command_text(f'{self.base_text}{text_write[0]}:  "{os.path.basename(audio_file[0])}"{NEW_LINE}')
                        self.inter.write_command_text(f'{self.base_text}{text_write[1]}:  "{os.path.basename(audio_file[1])}"{NEW_LINES}')
                    else:
                        self.inter.write_command_text(f'{self.base_text}{text_write[0]} & {text_write[1]} {SIMILAR_TEXT}{NEW_LINES}')
                        continue
                elif audio_tool_action == MANUAL_ENSEMBLE:
                    for n, i in enumerate(inputPaths):
                        self.inter.write_command_text(f'File {n+1} "{os.path.basename(i)}"{NEW_LINE}')
                    self.inter.write_command_text(NEW_LINE)
            ...
            if total_files == 1 and not is_verified_audio:
                self.inter.write_command_text(f'{error_text_console}\n{PROCESS_FAILED}')
                self.inter.write_command_text(time_elapsed())
                playsound(FAIL_CHIME) if self.is_task_complete_var.get() else None
            else:
                self.inter.write_command_text('{}{}'.format(process_complete_text, time_elapsed()))
                playsound(COMPLETE_CHIME) if self.is_task_complete_var.get() else None

            self.process_end()

        except Exception as e:
            self.inter.log_error(error_text(self.chosen_audio_tool_var.get(), e))
            self.inter.write_command_text(f'\n\n{PROCESS_FAILED}')
            self.inter.write_command_text(time_elapsed())
            playsound(FAIL_CHIME) if self.is_task_complete_var.get() else None
    ...
    def process_determine_secondary_model(self, process_method, main_model_primary_stem, is_primary_stem_only=False, is_secondary_stem_only=False):
        ...
        secondary_model = init_string(value=NO_MODEL)
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                if self.verify_audio(audio_file):
                    audio_file = self.create_sample(audio_file) if is_model_sample_mode else audio_file
                    if self.gui:
                        self.inter.write_command_text(f'{NEW_LINE if not file_num ==1 else NO_LINE}{base_text}"{os.path.basename(audio_file)}\".{NEW_LINES}')
                    is_verified_audio = True
                else:
                ...
                for current_model_num, current_model in enumerate(model, start=1):
                    ...
                    if is_ensemble:
                        self.inter.write_command_text(f'Ensemble Mode - {current_model.model_basename} - Model {current_model_num}/{len(model)}{NEW_LINES}')

                    model_name_text = f'({current_model.model_basename})' if not is_ensemble else ''
                    self.inter.write_command_text(base_text + f'{LOADING_MODEL_TEXT} {model_name_text}...')
                    ...
                    write_to_console = lambda progress_text, base_text=base_text:self.inter.write_command_text(base_text + progress_text)
                    ...
                    if is_ensemble:
                        self.inter.write_command_text('\n')

                if is_ensemble:

                    audio_file_base = audio_file_base.replace(f"_{current_model.model_basename}","")
                    self.inter.write_command_text(base_text + ENSEMBLING_OUTPUTS)
            ...
            if inputPath_total_len == 1 and not is_verified_audio:
                ...
                self.inter.write_command_text(PROCESS_COMPLETE)
                self.inter.write_command_text(time_elapsed())
                playsound(COMPLETE_CHIME) if self.is_task_complete_var.get() else None
            ...
            self.inter.log_error("{}{}".format(error_text(self.chosen_process_method_var.get(), e), self.get_settings_list()))
            if self.cli:
                print(f'\n\n{PROCESS_FAILED}')
                print(time_elapsed())
                raise e
    ...
    def load_to_default_confirm(self):
        ...
        if self.thread_check(self.active_processing_thread):
            self.inter.error_dialogue(SET_TO_DEFAULT_PROCESS_ERROR)
            return
    ...
    def load_saved_vars(self, data):
        ...
        ## ADD_BUTTON
        self.chosen_process_method_var = init_string(value=data['chosen_process_method'])

        #VR Architecture Vars
        self.vr_model_var = init_string(value=data['vr_model'])
        self.aggression_setting_var = init_string(value=data['aggression_setting'])
        self.window_size_var = init_string(value=data['window_size'])
        self.mdx_segment_size_var = init_string(value=data['mdx_segment_size'])
        self.batch_size_var = init_string(value=data['batch_size'])
        self.crop_size_var = init_string(value=data['crop_size'])
        self.is_tta_var = init_bool(value=data['is_tta'])
        self.is_output_image_var = init_bool(value=data['is_output_image'])
        self.is_post_process_var = init_bool(value=data['is_post_process'])
        self.is_high_end_process_var = init_bool(value=data['is_high_end_process'])
        self.post_process_threshold_var = init_string(value=data['post_process_threshold'])
        self.vr_voc_inst_secondary_model_var = init_string(value=data['vr_voc_inst_secondary_model'])
        self.vr_other_secondary_model_var = init_string(value=data['vr_other_secondary_model'])
        self.vr_bass_secondary_model_var = init_string(value=data['vr_bass_secondary_model'])
        self.vr_drums_secondary_model_var = init_string(value=data['vr_drums_secondary_model'])
        self.vr_is_secondary_model_activate_var = init_bool(value=data['vr_is_secondary_model_activate'])
        self.vr_voc_inst_secondary_model_scale_var = init_string(value=data['vr_voc_inst_secondary_model_scale'])
        self.vr_other_secondary_model_scale_var = init_string(value=data['vr_other_secondary_model_scale'])
        self.vr_bass_secondary_model_scale_var = init_string(value=data['vr_bass_secondary_model_scale'])
        self.vr_drums_secondary_model_scale_var = init_string(value=data['vr_drums_secondary_model_scale'])

        #Demucs Vars
        self.demucs_model_var = init_string(value=data['demucs_model'])
        self.segment_var = init_string(value=data['segment'])
        self.overlap_var = init_string(value=data['overlap'])
        self.overlap_mdx_var = init_string(value=data['overlap_mdx'])
        self.overlap_mdx23_var = init_string(value=data['overlap_mdx23'])
        self.shifts_var = init_string(value=data['shifts'])
        self.chunks_demucs_var = init_string(value=data['chunks_demucs'])
        self.margin_demucs_var = init_string(value=data['margin_demucs'])
        self.is_chunk_demucs_var = init_bool(value=data['is_chunk_demucs'])
        self.is_chunk_mdxnet_var = init_bool(value=False)
        self.is_primary_stem_only_Demucs_var = init_bool(value=data['is_primary_stem_only_Demucs'])
        self.is_secondary_stem_only_Demucs_var = init_bool(value=data['is_secondary_stem_only_Demucs'])
        self.is_split_mode_var = init_bool(value=data['is_split_mode'])
        self.is_demucs_combine_stems_var = init_bool(value=data['is_demucs_combine_stems'])#is_mdx23_combine_stems
        self.is_mdx23_combine_stems_var = init_bool(value=data['is_mdx23_combine_stems'])
        self.demucs_voc_inst_secondary_model_var = init_string(value=data['demucs_voc_inst_secondary_model'])
        self.demucs_other_secondary_model_var = init_string(value=data['demucs_other_secondary_model'])
        self.demucs_bass_secondary_model_var = init_string(value=data['demucs_bass_secondary_model'])
        self.demucs_drums_secondary_model_var = init_string(value=data['demucs_drums_secondary_model'])
        self.demucs_is_secondary_model_activate_var = init_bool(value=data['demucs_is_secondary_model_activate'])
        self.demucs_voc_inst_secondary_model_scale_var = init_string(value=data['demucs_voc_inst_secondary_model_scale'])
        self.demucs_other_secondary_model_scale_var = init_string(value=data['demucs_other_secondary_model_scale'])
        self.demucs_bass_secondary_model_scale_var = init_string(value=data['demucs_bass_secondary_model_scale'])
        self.demucs_drums_secondary_model_scale_var = init_string(value=data['demucs_drums_secondary_model_scale'])
        self.demucs_pre_proc_model_var = init_string(value=data['demucs_pre_proc_model'])
        self.is_demucs_pre_proc_model_activate_var = init_bool(value=data['is_demucs_pre_proc_model_activate'])
        self.is_demucs_pre_proc_model_inst_mix_var = init_bool(value=data['is_demucs_pre_proc_model_inst_mix'])

        #MDX-Net Vars
        self.mdx_net_model_var = init_string(value=data['mdx_net_model'])
        self.chunks_var = init_string(value=data['chunks'])
        self.margin_var = init_string(value=data['margin'])
        self.compensate_var = init_string(value=data['compensate'])
        self.denoise_option_var = init_string(value=data['denoise_option'])#
        self.phase_option_var = init_string(value=data['phase_option'])#
        self.phase_shifts_var = init_string(value=data['phase_shifts'])#
        self.is_save_align_var = init_bool(value=data['is_save_align'])#,
        self.is_match_silence_var = init_bool(value=data['is_match_silence'])#
        self.is_spec_match_var = init_bool(value=data['is_spec_match'])#
        self.is_match_frequency_pitch_var = init_bool(value=data['is_match_frequency_pitch'])#
        self.is_mdx_c_seg_def_var = init_bool(value=data['is_mdx_c_seg_def'])#
        self.is_invert_spec_var = init_bool(value=data['is_invert_spec'])#
        self.is_deverb_vocals_var = init_bool(value=data['is_deverb_vocals'])#
        self.deverb_vocal_opt_var = init_string(value=data['deverb_vocal_opt'])#
        self.voc_split_save_opt_var = init_string(value=data['voc_split_save_opt'])#
        self.is_mixer_mode_var = init_bool(value=data['is_mixer_mode'])
        self.mdx_batch_size_var = init_string(value=data['mdx_batch_size'])
        self.mdx_voc_inst_secondary_model_var = init_string(value=data['mdx_voc_inst_secondary_model'])
        self.mdx_other_secondary_model_var = init_string(value=data['mdx_other_secondary_model'])
        self.mdx_bass_secondary_model_var = init_string(value=data['mdx_bass_secondary_model'])
        self.mdx_drums_secondary_model_var = init_string(value=data['mdx_drums_secondary_model'])
        self.mdx_is_secondary_model_activate_var = init_bool(value=data['mdx_is_secondary_model_activate'])
        self.mdx_voc_inst_secondary_model_scale_var = init_string(value=data['mdx_voc_inst_secondary_model_scale'])
        self.mdx_other_secondary_model_scale_var = init_string(value=data['mdx_other_secondary_model_scale'])
        self.mdx_bass_secondary_model_scale_var = init_string(value=data['mdx_bass_secondary_model_scale'])
        self.mdx_drums_secondary_model_scale_var = init_string(value=data['mdx_drums_secondary_model_scale'])
        self.is_mdxnet_c_model_var = init_bool(value=False)

        #Ensemble Vars
        self.is_save_all_outputs_ensemble_var = init_bool(value=data['is_save_all_outputs_ensemble'])
        self.is_append_ensemble_name_var = init_bool(value=data['is_append_ensemble_name'])

        #Audio Tool Vars
        self.chosen_audio_tool_var = init_string(value=data['chosen_audio_tool'])
        self.choose_algorithm_var = init_string(value=data['choose_algorithm'])
        self.time_stretch_rate_var = init_string(value=data['time_stretch_rate'])
        self.pitch_rate_var = init_string(value=data['pitch_rate'])
        self.is_time_correction_var = init_bool(value=data['is_time_correction'])

        #Shared Vars
        self.semitone_shift_var = init_string(value=data['semitone_shift'])
        self.mp3_bit_set_var = init_string(value=data['mp3_bit_set'])
        self.save_format_var = init_string(value=data['save_format'])
        self.wav_type_set_var = init_string(value=data['wav_type_set'])#
        self.device_set_var = init_string(value=data['device_set'])#
        self.user_code_var = init_string(value=data['user_code'])
        self.is_gpu_conversion_var = init_bool(value=data['is_gpu_conversion'])
        self.is_primary_stem_only_var = init_bool(value=data['is_primary_stem_only'])
        self.is_secondary_stem_only_var = init_bool(value=data['is_secondary_stem_only'])
        self.is_testing_audio_var = init_bool(value=data['is_testing_audio'])#
        self.is_auto_update_model_params_var = init_bool(value=True)#
        self.is_auto_update_model_params = data['is_auto_update_model_params']
        self.is_add_model_name_var = init_bool(value=data['is_add_model_name'])
        self.is_accept_any_input_var = init_bool(value=data['is_accept_any_input'])
        self.is_task_complete_var = init_bool(value=data['is_task_complete'])
        self.is_normalization_var = init_bool(value=data['is_normalization'])#
        self.is_use_opencl_var = init_bool(value=False)#True if is_opencl_only else data['is_use_opencl'])#
        self.is_wav_ensemble_var = init_bool(value=data['is_wav_ensemble'])#
        self.is_create_model_folder_var = init_bool(value=data['is_create_model_folder'])
        self.help_hints_var = init_bool(value=data['help_hints_var'])
        self.model_sample_mode_var = init_bool(value=data['model_sample_mode'])
        self.model_sample_mode_duration_var = init_string(value=data['model_sample_mode_duration'])
        self.model_sample_mode_duration_checkbox_var = init_string(value=SAMPLE_MODE_CHECKBOX(self.model_sample_mode_duration_var.get()))
        self.model_sample_mode_duration_label_var = init_string(value=f'{self.model_sample_mode_duration_var.get()} Seconds')
        self.set_vocal_splitter_var = init_string(value=data['set_vocal_splitter'])
        self.is_set_vocal_splitter_var = init_bool(value=data['is_set_vocal_splitter'])#
        self.is_save_inst_set_vocal_splitter_var = init_bool(value=data['is_save_inst_set_vocal_splitter'])#

        #Path Vars
        self.export_path_var = init_string(value=data['export_path'])
        self.inputPaths = data['input_paths']
        self.lastDir = data['lastDir']

        #DualPaths-Align
        self.time_window_var = init_string(value=data['time_window'])#
        self.intro_analysis_var = init_string(value=data['intro_analysis'])
        self.db_analysis_var = init_string(value=data['db_analysis'])

        self.fileOneEntry_var = init_string(value=data['fileOneEntry'])
        self.fileOneEntry_Full_var = init_string(value=data['fileOneEntry_Full'])
        self.fileTwoEntry_var = init_string(value=data['fileTwoEntry'])
        self.fileTwoEntry_Full_var = init_string(value=data['fileTwoEntry_Full'])
